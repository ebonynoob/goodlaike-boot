...
public class SmsUtil {
   ...
   * 发送短信
   * 
   * @param aliSmsType 阿里短信类型
   * @param recNum 接收号码，多个号码用 , 间隔
   * @param params 短信模板中用到的参数
  ...
  public static void sendSms(AliSmsType aliSmsType, String recNum, String... params) throws ClientException {
    ...
    try {
      ...
      logger.info("==>> 发送短信[" + request.getParamString() + "]");
      // SingleSendSmsResponse httpResponse =
      // client.getAcsResponse(request);
      // System.out.println(httpResponse);
    } catch (ClientException e) {
      logger.error("==>> 发送短信失败[" + request.getParamString() + "]，ex：[" + e + "]");
      e.printStackTrace();
    }
  }
